/**
 * Harnais partagé pour charger de VRAIS `.glb` (compressés Draco) hors
 * navigateur, extrait de `propAssets.test.ts` (#51) pour que `parkAssets.test.ts`
 * (#58) le réutilise sans dupliquer ~270 lignes de code de décodage.
 *
 * Charger ces `.glb` hors navigateur pose deux problèmes que ni
 * `propAssetsResource` ni `parkAssetsResource` n'ont à résoudre :
 *
...
function typeDracoPour(draco: DracoModule, TypedArray: ConstructeurTypedArray): number {
  switch (TypedArray) {
    ...
    default: throw new Error('glbTestHarness : type de tableau draco inattendu')
  }
}
...
function decoderGeometrieDraco(draco: DracoModule, decodeur: DracoDecodeur, tableau: Int8Array, tache: TacheDecodage) {
  ...
  if (type !== draco.TRIANGULAR_MESH) throw new Error('glbTestHarness : géométrie non triangulaire inattendue')
  const statut = decodeur.DecodeArrayToMesh(tableau, tableau.byteLength, maillage)
  if (!statut.ok() || maillage.ptr === 0) throw new Error('glbTestHarness : décodage draco échoué — ' + statut.error_msg())
...
/**
 * À appeler dans un `beforeAll` : relie `DRACOLoader.decodeGeometry` sur
 * draco3d (thread principal, sans Worker) et court-circuite `ImageLoader`
 * (jsdom ne déclenche jamais `onload` sur une vraie `<img>`).
 */
export async function installerDecodeurDraco(): Promise<void> {
...
export async function chargerGLTF(cheminRelatif: string) {
...
export function geometriesMonde(noeud: THREE.Object3D): THREE.BufferGeometry[] {
 ...
 */
export function fusionnerPositions(geometries: readonly THREE.BufferGeometry[]): THREE.BufferGeometry {
...
export function centreXZ(pieces: readonly { geometry: THREE.BufferGeometry }[]): THREE.Vector3 {
